...
void main() {
  group('PlugCard', () {
    ...
    group('icon rendering', () {
      testWidgets(
        'online + on state renders green power icon',
        (tester) async {
          final device = TestFixtures.onlineDevice();
          await tester.pumpWidget(
            buildTestWidget(device),
          );
          ...
          final icon =
              tester.widget<Icon>(iconFinder);
          expect(icon.color, Colors.green);
        },
      );

      testWidgets(
        'online + off state renders grey power_off icon',
        (tester) async {
          final device = TestFixtures.onlineDevice(
            deviceOn: false,
          );
      ...
      testWidgets(
        'offline state renders error-colored icon',
        (tester) async {
    ...
    group('device info display', () {
      ...
      testWidgets('displays IP address',
          (tester) async {
        final device = TestFixtures.onlineDevice();
        await tester.pumpWidget(
          buildTestWidget(device),
        ...
        expect(
          find.text(TestFixtures.testDeviceIp),
          findsOneWidget,
        );
      });

      testWidgets(
        'displays Unknown Device for empty nickname',
        (tester) async {
      ...
      testWidgets(
        'displays Tapo Plug for empty model',
        (tester) async {
          const device = TapoDevice(
            ip: '192.168.1.100',
            nickname: 'Test',
    ...
    group('isToggling state', () {
      testWidgets(
        'shows spinner when toggling, switch otherwise',
        (tester) async {
          final device = TestFixtures.onlineDevice();
    ...
    group('switch interaction', () {
      testWidgets(
        'switch tap calls onToggle for online device',
        (tester) async {
          final device = TestFixtures.onlineDevice(
            deviceOn: false,
          );
      ...
      testWidgets(
        'switch is disabled for offline device',
        (tester) async {
          final device = TestFixtures.offlineDevice();
          await tester.pumpWidget(
            buildTestWidget(device),
      ...
      testWidgets(
        'switch reflects device on/off state',
        (tester) async {
          // Device on
          await tester.pumpWidget(
            buildTestWidget(
    ...
    group('dismissible (swipe to delete)', () {
      ...
      testWidgets(
        'dismiss triggers confirm dialog',
        (tester) async {
          final device = TestFixtures.onlineDevice(
            nickname: 'My Plug',
          );
      ...
      testWidgets(
        'cancel in dialog prevents removal',
        (tester) async {
          final device = TestFixtures.onlineDevice();
          await tester.pumpWidget(
            buildTestWidget(device),
      ...
      testWidgets(
        'confirm in dialog calls onRemove',
        (tester) async {
          final device = TestFixtures.onlineDevice();
          await tester.pumpWidget(
            buildTestWidget(device),
    ...
    group('card structure', () {
      testWidgets(
        'renders Card with device IP as key',
        (tester) async {
          final device = TestFixtures.onlineDevice(
            ip: '10.0.0.5',
          );
